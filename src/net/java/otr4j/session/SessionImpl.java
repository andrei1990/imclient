...
public class SessionImpl implements Session {
	...
	private void setSessionStatus(SessionStatus sessionStatus)
		...
		if (sessionStatus == SessionStatus.ENCRYPTED && doTransmitLastMessage
				&& lastSentMessage != null) {
			String msg = this.transformSending(
					(isLastMessageRetransmit ? "[resent] " : "")
							+ lastSentMessage, null);
	...
	public String transformReceiving(String msgText) throws OtrException {
		...
		case AbstractEncodedMessage.MESSAGE_SIGNATURE:
			Log.i("handshake", "message signature");
			AuthContext auth = this.getAuthContext();
			...
			if (auth.getIsSecure()) {
				...
				ImOtrApp.WAS_CONVERSATION_ENCRYPTED=true;
				Log.i("handshake", "sender name is " + sessionId.getUserID());
				ImOtrApp.initiatedSessions.put(sessionId.getUserID(), sessionId.getUserID());
				Intent encryptedConversation = new Intent(
						FriendList.ENCRYPTED_CONVERSATION);
				encryptedConversation.putExtra(FriendList.SENDER_NAME, sessionId.getUserID());
				context.sendBroadcast(encryptedConversation);
				Log.i("handshake", "secured conversation");
				// String test = "test";
				// String message = this.transformSending(test, null);
				// this.getHost().injectMessage(getSessionID(), message);
				// logger.finest("Gone Secure.");
			} else {
				Log.i("handshake", " not secured conversation");
			}
			return null;
		default:
	...
	private void handleQueryMessage(QueryMessage queryMessage)
		...
		if (queryMessage.versions.contains(2) && policy.getAllowV2()) {
			...
			Intent intent=new Intent(FriendList.SHOW_QUERY_DIALOG);
		...
		case ENCRYPTED:
			...
			if (!Arrays.equals(computedMAC, data.mac)) {
				throw new OtrException(
						"MAC verification failed, ignoring message");
			}
			...
			Log.i("handshake", "decrypted message content: "
					+ decryptedMsgContent);
	...
	public void injectMessage(AbstractMessage m) throws OtrException {
		...
		SessionID sesesion = getSessionID();
		Log.i("test", sesesion.getAccountID());
		Log.i("test", sesesion.getUserID());
