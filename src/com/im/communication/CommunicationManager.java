...
public class CommunicationManager implements ICommunicationManager {
	...
	// "http://10.0.2.2:8080/ServerIM/IMServer";
	private static final String AUTHENTICATION_SERVER_ADDRESS = "http://serverim.jelastic.dogado.eu/serverIm/IMServer";
	...
	private class ReceiveConnection extends Thread {
		...
		public ReceiveConnection(Socket socket) {
			...
			CommunicationManager.this.sockets.put(socket.getInetAddress(), socket);
		}
			...
			try {
				...
				while ((inputLine = in.readLine()) != null) {
					if (inputLine.equals("exit") == false) {
						...
						clientSocket.close();
						CommunicationManager.this.sockets.remove(clientSocket
								.getInetAddress());
					}
				}
	...
	public CommunicationManager(IAppManager appManager) {
	...
	private Socket getSocket(InetAddress addr, int portNo) {
		Socket socket = null;
		// check if the map contains the socket for the given IP address
